...
export default function RotatePdfTool() {
  ...
  const handleRotate = async () => {
    ...
    const blob = await new Promise<Blob | null>((resolve) => {
      ...
      xhr.open("POST", `${apiBase}/api/pdf/rotate`);
      applyAuthHeader(xhr);
      ...
      xhr.onload = () => {
        if (xhr.status < 200 || xhr.status >= 300) {
          setStatus(xhr.responseText || "Rotate failed.");
          resolve(null);
          return;
        }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
      <div className="glass-card rounded-2xl p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h3 className="mt-2 text-lg font-semibold text-ink-950">Rotate pages</h3>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-obsidian-200 bg-obsidian-50 p-4 text-sm text-obsidian-500">
          {file ? `${file.name} ready.` : "Upload a PDF to rotate pages."}
        </div>
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        <div className="mt-4 grid gap-4 text-sm text-ink-900">
          <div className="rounded-2xl bg-white px-4 py-3">Rotation is applied to selected pages.</div>
          <Button variant="premium" onClick={handleRotate} disabled={!file}>
            Rotate PDF
          </Button>
