...
export default function PdfToPagesTool() {
  ...
  const handleConvert = async () => {
    if (!file) return;
    setStatus("Converting to Pages... (macOS only)");
    setProgress(0);
    const formData = new FormData();
    formData.append("file", file);
    const blob = await new Promise<Blob | null>((resolve) => {
      const xhr = new XMLHttpRequest();
      xhr.open("POST", `${apiBase}/api/pdf/pdf-to-pages`);
      applyAuthHeader(xhr);
    ...
    await saveHistoryItem({
      name: `pdf-to-pages-${new Date().toISOString().slice(0, 10)}`,
      blob,
      kind: "pages"
    });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "converted.pages";
    link.click();
    URL.revokeObjectURL(link.href);
    setStatus("Pages file downloaded.");
      ...
      <div className="glass-card rounded-2xl p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h3 className="mt-2 text-lg font-semibold text-ink-950">PDF to Pages</h3>
          </div>
        ...
        >
          {file ? `${file.name} ready for conversion.` : "Upload a PDF to convert to Pages."}
        </div>
      </div>
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        <div className="mt-4 grid gap-4 text-sm text-ink-900">
          <div className="rounded-2xl bg-white px-4 py-3">
            macOS only: requires Pages installed on the API host.
          </div>
