...
export default function RemovePagesTool() {
  ...
  const handleRemove = async () => {
    ...
    const blob = await new Promise<Blob | null>((resolve) => {
      ...
      xhr.open("POST", `${apiBase}/api/pdf/remove`);
      applyAuthHeader(xhr);
      ...
      xhr.onload = () => {
        if (xhr.status < 200 || xhr.status >= 300) {
          setStatus(xhr.responseText || "Remove failed.");
          resolve(null);
          return;
        }
    ...
    await saveHistoryItem({
      name: `removed-pages-${new Date().toISOString().slice(0, 10)}`,
      blob,
      kind: "pdf"
    });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "removed-pages.pdf";
    link.click();
    URL.revokeObjectURL(link.href);
    setStatus("Updated PDF downloaded.");
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
      <div className="glass-card rounded-2xl p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h3 className="mt-2 text-lg font-semibold text-ink-950">Remove pages</h3>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-obsidian-200 bg-obsidian-50 p-4 text-sm text-obsidian-500">
          {file ? `${file.name} ready.` : "Upload a PDF to remove pages."}
        </div>
        <div className="mt-4 grid gap-4 text-sm text-ink-900">
          ...
          <div className="text-xs text-obsidian-500">Example: 2,4,6 or 1-3</div>
        </div>
      </div>
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        <div className="mt-4 grid gap-4 text-sm text-ink-900">
          <div className="rounded-2xl bg-white px-4 py-3">Removed pages are excluded from the output.</div>
          <Button variant="premium" onClick={handleRemove} disabled={!file}>
            Remove Pages
          </Button>
